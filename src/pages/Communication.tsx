...
export default function Communication() {
  ...
  return (
    <div className="space-y-8">
      ...
      <div className="space-y-4">
        {filteredMessages.map((msg) => (
          <Card key={msg.id} className="bg-[#1e3a8a] text-white border-0 p-6 hover:bg-[#1e3a8a]/90 transition-all">
            <div className="flex gap-5">
              <div className={`w-12 h-12 rounded-2xl flex-shrink-0 flex items-center justify-center ${msg.type === 'sms' ? 'bg-blue-500/20 text-blue-300' : 'bg-purple-500/20 text-purple-300'}`}>
                {msg.type === 'sms' ? <MessageSquare size={26} /> : <Phone size={26} />}
              </div>

              <div className="flex-1 min-w-0">
                <div className="flex justify-between items-start">
                  <p className="font-semibold text-white">{msg.patient}</p>
                  <p className="text-xs text-white/70 whitespace-nowrap ml-4">{msg.time}</p>
                </div>

                {/* Prominent Message Content */}
                <p className="text-white/90 mt-3 text-[15.5px] leading-relaxed font-medium">
                  {msg.content}
                </p>
                ...
                <div className="flex items-center gap-3 mt-4">
                  ...
                  />
                  <span className="text-xs text-white/70 capitalize font-medium">{msg.direction}</span>
                </div>
              </div>
            </div>
          </Card>
